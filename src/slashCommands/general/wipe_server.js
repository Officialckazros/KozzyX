...
export default {
    ...
    async execute(interaction) {
        ...
        const lines = [
            `Deleted **${deletedChannels}** channel(s) and categor(ies).`,
            `Deleted **${deletedRoles}** role(s).`,
            `Cleared bot settings.`,
            skipped.length ? `Skipped: ${skipped.slice(0, 5).join(", ")}${skipped.length > 5 ? ` +${skipped.length - 5} more` : ""}` : null,
        ].filter(Boolean);

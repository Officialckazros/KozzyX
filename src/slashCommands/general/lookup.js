...
export async function getLookupResponse(client, executorGuildId, targetUserId, executorUser, page = 0) {
    ...
    if (page === 0 || page === 1) {
        const fetchPromises = Array.from(client.guilds.cache.values()).map(async (guild) => {
            try {
                let m = guild.members.cache.get(targetUserId);
                if (!m) {
                    m = await guild.members.fetch(targetUserId).catch(() => null);
                }
                if (m) {
    ...
    let globalModCases = [];
    let globalModCasesCount = 0;
    if (page === 0 || page === 2) {
        globalModCases = await db.all("SELECT guild_id, case_number, action, executor_id, reason, duration_ms, created_at FROM mod_cases WHERE target_id = ? ORDER BY created_at DESC", targetUserId).catch(() => []);
        globalModCasesCount = globalModCases.length;
    }

    let globalAppeals = [];
    let globalAppealsCount = 0;
    if (page === 2) {
        globalAppeals = await db.all("SELECT guild_id, id, status, reason, created_at FROM appeals WHERE user_id = ? ORDER BY created_at DESC", targetUserId).catch(() => []);
        globalAppealsCount = globalAppeals.length;
    }

    let todoCount = 0;
    let reminderCount = 0;
    let afkInfo = null;
    let cosmeticsInfo = null;
    ...
    let inviteJoins = [];
    let invitesGrouped = [];
    let totalInvitesGroupedCount = 0;
    const activeInvitesCreated = [];
    let ownedDynamicVCs = [];
    let auditLogs = [];

    if (page === 3) {
        todoCount = await db.get("SELECT COUNT(*) AS c FROM todos WHERE user_id = ?", targetUserId).then(r => r?.c ?? 0).catch(() => 0);
        reminderCount = await db.get("SELECT COUNT(*) AS c FROM reminders WHERE user_id = ?", targetUserId).then(r => r?.c ?? 0).catch(() => 0);
        afkInfo = afkMap.get(targetUserId);
        cosmeticsInfo = cosmeticsMap.get(targetUserId);
        boosterRoleId = boosterRolesDB.get(targetUserId);
        birthdaysList = await db.all("SELECT guild_id, birthday_date FROM birthdays WHERE user_id = ?", targetUserId).catch(() => []);
        ...
        const fetchPromises = Array.from(client.guilds.cache.values()).map(async (guild) => {
            try {
                ...
                if (m) {
                    mutualGuildsForInvites.push(guild);
                }
            } catch {}
        });
        await Promise.all(fetchPromises);

        const invitePromises = mutualGuildsForInvites.map(async (guild) => {
            try {
                const memberMe = guild.members.me;
        ...
        ownedDynamicVCs = await db.all("SELECT channel_id, guild_id, created_at FROM dynamic_vcs WHERE owner_id = ?", targetUserId).catch(() => []);
        auditLogs = await db.all(
    ...
    if (page === 0) {
        ...
        if (isExtremelyNew) {
            ...
            riskFactors.push(`Account created ${accountAgeDays} days ago (new)`);
        }
        ...
        if (globalWarningCount >= 3) {
            ...
            riskFactors.push(`Active warnings (${globalWarningCount} globally)`);
        }
        ...
        if (activeModInfractions >= 2) {
            ...
            riskFactors.push(`Global mod history (${activeModInfractions} action(s))`);
        }
        ...
        const isNewAccount = accountAgeDays < 30;
        const isExtremelyNew = accountAgeDays < 7;
        const activeModInfractions = globalModCases ? globalModCases.filter(c => ["ban", "kick", "timeout", "softban"].includes(c.action.toLowerCase())).length : 0;

        if (isExtremelyNew || globalWarningCount >= 3 || activeModInfractions >= 2) {
            riskLevel = "High Risk";
        } else if (isNewAccount || (user && !user.avatar) || globalWarningCount > 0 || activeModInfractions > 0) {
            riskLevel = "Medium Risk";
        }
    }
    ...
    if (page === 0) {
        ...
        fields.push({ name: `Security Risk Assessment: ${riskLevel}`, value: riskValue, inline: false });
        ...
        if (globalWarningCount > 0) {
            ...
            fields.push({ name: "Global Warning History", value: warnText.slice(0, 1024), inline: false });
        } else {
            fields.push({ name: "Global Warning History", value: "• *Clean record (zero warnings globally).* ", inline: false });
        }
        ...
        if (activeInvitesCreated.length > 0) {
            const act = activeInvitesCreated.slice(0, 3).map(inv => {
                const exp = inv.expiresTimestamp ? `<t:${Math.floor(inv.expiresTimestamp / 1000)}:R>` : "Permanent ";
        ...
        if (auditLogs.length > 0) {
            ...
            fields.push({ name: "Recent Audit Log Activity", value: logsText.slice(0, 1024), inline: false });
        }
